...
RUSTIO_DOCKERFILE="${RUSTIO_DOCKERFILE:-Dockerfile.rustio}"
...
RUSTIO_MEMORY_TRIM_PERIODIC_SECONDS="${RUSTIO_MEMORY_TRIM_PERIODIC_SECONDS:-600}"
...
RUSTIO_STORAGE_SCAN_INTERVAL_MS="${RUSTIO_STORAGE_SCAN_INTERVAL_MS:-1800000}"
...
prepare_environment() {
  ...
  if [[ "${DOCKER_BUILDKIT:-1}" == "0" ]] && [[ "$RUSTIO_DOCKERFILE" == "Dockerfile.rustio" ]]; then
    RUSTIO_DOCKERFILE="Dockerfile.rustio.legacy"
    log "检测到 DOCKER_BUILDKIT=0，已切换到兼容 Dockerfile：$RUSTIO_DOCKERFILE"
  fi
  export RUSTIO_DOCKERFILE
  ...
  export RUSTIO_MEMORY_TRIM_PERIODIC_SECONDS
...
run_compose() {
  ...
  log "构建文件：$RUSTIO_DOCKERFILE"
  log "内存回收：enabled=$RUSTIO_MEMORY_TRIM_ENABLED idle=${RUSTIO_MEMORY_TRIM_IDLE_SECONDS}s periodic=${RUSTIO_MEMORY_TRIM_PERIODIC_SECONDS}s force=${RUSTIO_MEMORY_TRIM_FORCE_INTERVAL_SECONDS}s rss=${RUSTIO_MEMORY_TRIM_RSS_THRESHOLD_MB}MB"
