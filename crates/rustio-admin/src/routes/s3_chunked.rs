...
///
/// 持有链式状态：上一块签名（初始为 seed）、signing_key、date/scope/region。
/// 每块：解析帧头 → 读 payload（上限 16 MiB）→ 验签（链式依赖上一签名）→ 更新状态。
...
impl<R: tokio::io::AsyncRead + Unpin> AwsChunkedDecoder<R> {
    /// 构造解码器（从 `StreamingSigV4Context` 初始化链式状态）。
    pub(crate) fn new(inner: R, ctx: crate::routes::s3_sign::StreamingSigV4Context) -> Self {
        Self {
            inner,
    ...
    async fn parse_chunk_header(&mut self) -> Result<(usize, String), std::io::Error> {
        self.line_buf.clear();
        loop {
        ...
        let Some((size_hex, rest)) = line.split_once(';') else {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidData,
                "missing ';' in chunk header",
            ));
        };
        ...
        Ok((chunk_size, sig.to_string()))
    }

    /// 验证块签名（链式：string_to_sign 含上一块签名 + 当前块 hash）。
    fn verify_chunk_signature(
        ...
        let string_to_sign = format!(
            ...
            self.amz_date, self.credential_scope, self.prev_signature, EMPTY_SHA256, chunk_hash
        );

        let computed_sig = hmac_sha256_hex(&self.signing_key, &string_to_sign);
